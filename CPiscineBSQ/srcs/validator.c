...
/*   Updated: 2022/09/12 23:30:20 by soulee           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	valid_lines(char **splited_file_contents)
{
	int		i;
	int		lines;
	char	*characters;
	...
	count_first_line = ft_strlen(splited_file_contents[0]);
	characters = get_string(splited_file_contents[0],
			count_first_line - 3, 2);
	lines = ft_atoi(get_string(splited_file_contents[0],
				0, count_first_line - 3));
	i = 1;
	count_before_line = ft_strlen(splited_file_contents[1]);
	while (i < lines)
	{
		is_valid_string(splited_file_contents[i], characters);
		if (count_before_line != ft_strlen(splited_file_contents[i]))
			print_error("ERROR: VALIDATOR (LINE LENGTH NOT MATCHING)\n");
		count_before_line = ft_strlen(splited_file_contents[i]);
		i++;
	}
}
...
void	valid_file(char **splited_file_contents)
{
	...
	count = count_tab(splited_file_contents);
	count_first_line = ft_strlen(splited_file_contents[0]);
	lines = ft_atoi(get_string(splited_file_contents[0],
				0, count_first_line - 3));
	if (count <= 0)
	...
	if (!is_numeric(get_string(splited_file_contents[0],
				0, count_first_line - 3)))
		print_error("ERROR: VALIDATOR (NOT NUMBER LINE ARGUMENT)\n");
	if (!is_printable(get_string(splited_file_contents[0],
	...
	valid_lines(splited_file_contents);
}
